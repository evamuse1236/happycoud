...
test('generic reactions are excluded while specific jokes survive',()=>{
  for(const t of ['❤️😍','Niiice','Stunningggg omg','uWu','HOTTAAYYYY','Pretty as usual 😍','Omggggg so pretty'])assert.equal(isGeneric(t),true,t);
  for(const t of ['We should do a swayamvar','Best girl in the best park','Saxy hairs deers.','Mary. Me deer.','What is your life purpose?','Your smile is just so genuine and beautiful❤'])assert.equal(isGeneric(t),false,t);
